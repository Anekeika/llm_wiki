...
def append_to_daily_log(content: str, section: str = "Session") -> None:
    ...
    entry = f"### {section} ({time_str}) [{device}]\n\n{content}\n\n"
...
def git_push() -> None:
    """Commit all wiki changes and push to GitHub, tagging the device name."""
    device = socket.gethostname()
    now = datetime.now(timezone.utc).astimezone()
    timestamp = now.strftime("%Y-%m-%d %H:%M")
    commit_msg = f"auto: sync [{device}] {timestamp}"

    try:
        # Stage all changes in the wiki repo
        subprocess.run(
            ["git", "add", "-A"],
            cwd=str(WIKI_DIR),
            capture_output=True,
        ...
        if result.returncode != 0 and "nothing to commit" not in result.stdout + result.stderr:
            logging.warning("git commit: %s", result.stderr.strip())
            return
        # Push
        push = subprocess.run(
            ["git", "push"],
        ...
        if push.returncode != 0:
            logging.warning("git push failed: %s", push.stderr.strip())
        else:
            logging.info("git push OK: %s", commit_msg)
    except Exception as e:
        logging.warning("git_push error: %s", e)
...
def main():
    ...
    # Commit and push all wiki changes to GitHub so other devices stay in sync.
    git_push()
